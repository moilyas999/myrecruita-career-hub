  ...
  try {
    ...
    if (type === "magic_link" || type === "otp") {
      ...
      const { data, error } = await supabase.auth.admin.generateLink({
        ...
        email: email,
        options: {
          redirectTo: redirectUrl || `${supabaseUrl.replace('.supabase.co', '.lovable.app')}/dashboard`,
        },
      });
      ...
      // Extract OTP from properties
      code = data.properties?.email_otp || generateOTP();
      link = data.properties?.action_link;
      
      console.log("Generated auth link for:", email, "Code:", code ? "generated" : "none");
    } else if (type === "password_reset") {
      const { data, error } = await supabase.auth.admin.generateLink({
        type: "recovery",
        email: email,
        options: {
          redirectTo: redirectUrl || `${supabaseUrl.replace('.supabase.co', '.lovable.app')}/auth?type=recovery`,
        },
      });
    ...
    return new Response(
      JSON.stringify({ 
        ...
        // Return the code for OTP verification (securely - don't log it)
